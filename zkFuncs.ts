...
const executeCircuit = async (circuit: any, inputs: any) => {
...
const getSignalByName = (circuit: any, witness: any, signal: string) => {
  return witness[circuit.symbols[signal].varIdx];
};

const genBatchUstProofAndPublicSignals = (inputs: any, circuit?: any) => {
...
const genQvtProofAndPublicSignals = (inputs: any, circuit?: any) => {
  return genProofAndPublicSignals(
    inputs,
...
const genProofAndPublicSignals = async (
  ...
  const inputJsonPath = path.join(
    __dirname,
    "../build/" + date + ".input.json"
  );
  const witnessPath = path.join(
    __dirname,
    "../build/" + date + ".witness.wtns"
  );
  const witnessJsonPath = path.join(
    __dirname,
    "../build/" + date + ".witness.json"
  );
  const proofPath = path.join(__dirname, "../build/" + date + ".proof.json");
  const publicJsonPath = path.join(
    __dirname,
    "../build/" + date + ".publicSignals.json"
  );
...
const verifyBatchUstProof = (proof: any, publicSignals: any) => {
  return verifyProof("batchUstVk.json", proof, publicSignals);
};

const verifyQvtProof = (proof: any, publicSignals: any) => {
